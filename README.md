...
`7` is a 240×180 GPU point-matter height field: 43,200 persistent microscopic samples, with no carrier plane, glyph quads or 0/1 texture. Its 14×12 simulation domain is larger than the visible field; irregular opacity, emission, point-size, density and distance dissolution hide the physical boundaries. Four-octave macro deformation, nine warped non-parallel interference components and six evolving local pressure systems form an active coherent 2D field. TERRAIN uses a near-horizontal state camera looking across the field. Entry has explicit phases `convergeSource → sourceHold → releasePoints → propagate → idle`: states 1–5 first become compact Kernel+Chaos, while CUBE reuses its existing reverse collapse through `reverseSeedOnly`. Persistent points then travel from that source to their `(x,H(x,z,t),z)` coordinates behind a distorted pressure front. Exit reverses those coordinates into the same source. The palette remains graphite, smoked silver, warm ivory, restrained amber and rare vermilion. Runtime inspection is available through `getTerrainPhase()`, `getTerrainParameters()` and `setTerrainParameter(name, value)` on `window.__coreVisualDebug`. The Git visual is unchanged except for `localEventFrequency = 3.75`, three times the original wave speed.
